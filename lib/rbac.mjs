...
export function getVisibleNavigationItems(user) {
    ...
    return allItems.filter((item) => roleHasPermission(user, getNavigationPermission(item.href)))
}
...
function getNavigationPermission(href) {
    ...
    if (href === '/purchases') return 'purchases'
    if (['/bank-txn', '/banks', '/daily-close', '/ledger', '/receivables', '/payables', '/prepayments', '/supplier-rebates', '/loans', '/tax'].includes(href)) return 'accounting'
