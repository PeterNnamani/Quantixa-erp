...
export default function InventorySheetTable({ sheet, onSheetChange, inventory, purchases, sales, auditLogs, supplierList, search = '' }: InventorySheetTableProps) {
  ...
  const products = inventory.map((item, index) => ({
    ...
    supplier: item.supplier || supplierList[index % Math.max(supplierList.length, 1)] || '-',
    active: item.active ?? true,
  }))

  if (sheet === 'product-master') {
    const rows = products.filter((item) => matches([item.rowId, item.product, item.brand, item.dept, item.supplier]))
    return <SheetFrame title="Product Master" count={rows.length} sheet={sheet} onSheetChange={onSheetChange} headers={['SKU', 'Product Name', 'Brand', 'Category', 'Pack Size', 'Unit Cost', 'Selling Price', 'Reorder Level', 'Reorder Quantity', 'Supplier', 'Status']}>
      {rows.map((item) => <tr key={item.rowId}><td>{item.rowId}</td><td>{item.product}</td><td>{item.brand}</td><td>{item.dept || '-'}</td><td>{item.packSize}</td><td>{formatCurrency(item.unitCost)}</td><td>{formatCurrency(item.sellingPrice ?? item.unitCost * 1.35)}</td><td>{item.reorderLevel}</td><td>{item.reorderQuantity}</td><td>{item.supplier}</td><td><Status value={item.active ? 'Active' : 'Inactive'} /></td></tr>)}
    </SheetFrame>
  }
