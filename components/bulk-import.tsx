    ...
    const upload = async () => {
        ...
        try {
            ...
            const response = await fetch('/api/import', {
                ...
                body: JSON.stringify({ ...prepared.payload, companyId: user.companyId, staffId: user.staffId }),
            })
            const result = await response.json()
            if (!response.ok || !result.success) throw new Error(result.error || 'The database rejected this import.')
            ...
            updateState({
                ...
                supplierList: Array.from(new Set([...state.supplierList, ...prepared.payload.contacts.filter((contact: any) => contact.type === 'supplier').map((contact: any) => String(contact.name))])),
            })
    ...
    return <>
        ...
        {open && <div className="bulk-import-backdrop" role="dialog" aria-modal="true" aria-label="Bulk upload">
            <section className="bulk-import-modal">
                ...
                <div className="btn-group"><button className="btn btn-secondary" type="button" onClick={close}>Cancel</button><button className="btn btn-primary" type="button" disabled={!rows.length || busy} onClick={() => void upload()}>{busy ? 'Saving...' : 'Analyze and save'}</button></div>
            </section>
        </div>}
    </>
}
